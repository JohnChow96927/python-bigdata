...
refund_base as(
    select *,
       row_number() over(partition by order_id, goods_id) rn
    from yp_dwb.dwb_order_detail
    where dt='${TD_DATE}' and refund_id is not null
),
-- 退款次数、件数、金额
refund_count as (
    select dt, goods_id sku_id, goods_name sku_name,
...
cart_count as (
    ...
    where cart.start_date='${TD_DATE}' and cart.end_date = '9999-99-99'
    group by substring(cart.create_time, 1, 10), goods_id
),
-- 收藏次数
favor_count as (
    ...
    where c.start_date='${TD_DATE}' and c.end_date='9999-99-99'
...
store_collect_count as (
   ...
   where end_date='9999-99-99' and substring(create_time, 1, 10)='${TD_DATE}'
...
cart_count_amount as (
   ...
   where cart.end_date='9999-99-99' and cart.goods_id=g.id and dt='${TD_DATE}'
...
(
	select
		buyer_id, create_date, order_id, order_amount,
		row_number() over(partition by orderd.order_id) rn
	from yp_dwb.dwb_order_detail orderd
	where orderd.is_valid=1 and dt='${TD_DATE}'
),
-- 下单次数和金额
order_count_amount as (
	select
		o.buyer_id user_id, substring(o.create_date,1,10) dt,
...
payment_count_amount as (
   ...
   where is_valid = 1 and pay_time is not null and end_date='9999-99-99' and start_date='${TD_DATE}'
