...
SELECT
    ...
    CONCAT(ROUND(SUM(quantity * unit_price) / SUM(SUM(quantity * unit_price)) OVER(PARTITION BY month) * 100, 2), '%') AS `ratio`
FROM dim_orders
GROUP BY month, product_id;
...
SELECT
    ...
    CONCAT(ROUND((month_revenue - LAG(month_revenue) OVER(ORDER BY month)) / LAG(month_revenue) OVER(ORDER BY month) * 100, 2), '%') AS `ratio`
...
WITH month_order_users AS (
    -- 对同一个用户同一个月的订单进行去重
    SELECT
        DISTINCT MONTH(order_datetime) AS `month`,
        user_id
    FROM fact_order_detail
), tb_temp1 AS (
    SELECT
