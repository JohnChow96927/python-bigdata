package a_date;

import java.text.SimpleDateFormat;
import java.util.Date;

public class D_Date_Utils {
    public static void main(String[] args) throws Exception{
        // 目标1: 将时间戳转成指定格式的时间字符串
        long timestamp = 1650245131000L;
        // 分解式
        Date d1 = new Date(timestamp);

        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String result = sdf.format(d1);

        // 合并式
        String result2 = timestamp2datestr(timestamp);

        // 目标2: 将指定格式的时间字符串转成时间戳
        String str = "2022-04-18 21:53:14";

        // 分解式
        SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date d2 = sdf2.parse(str);
        long timestamp2 = d2.getTime();
        System.out.println(timestamp2);

        // 合并式
        long timestamp3 = datestr2timestamp(str);
        System.out.println(timestamp3);
    }

    private static long datestr2timestamp(String str) throws Exception {
        return new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(str).getTime();
    }

    private static String timestamp2datestr(long timestamp) {
        return new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(timestamp));
    }
}
