...
- **规则**
  ...
  - ==消费者：根据offset==来持续消费，只要**保证任何场景下消费者都能知道上一次的Offset**即可

- **实现**

  - 将**offset存储在一种可靠外部存储中，手动管理offset**
...
- **步骤**
  ...
  - step3：处理成功：将处理成功的分区的Offset进行额外的存储
    ...
    - Kafka：默认存储`__consumer_offsets`
    - 外部：`MySQL、Redis、Zookeeper`

    ![1636013487572](assets/1636013487572.png)
