...
- 分布式存储系统的**四大属性**：
  ...
  - **副本机制**：硬件故障难以避免，数据丢失风险大，不同机器设置备份，冗余存储保障数据安全。**ES纠删码**
...
2. ### HDFS重要特性
   ...
   2. #### 分块存储
      ...
      ##### 	HDFS中的文件在物理上是分块存储（block）的，块的大小可以通过配置参数来规定，默认大小在hadoop2.x版本中是128M。并行计算
...
3. ### HDFS基本操作
   ...
   1. #### Shell命令行客户端
      ...
      URI格式为scheme://authority/path。对于HDFS，该scheme是hdfs，对于本地FS，该scheme是file。scheme和authority是可选的。如果未指定，则使用配置中指定的默认方案，如果配置中未配置则使用本地文件系统。
...
3. ### HDFS的工作机制
   ...
   1. #### HDFS写数据流程（上传）
      ...
      ```properties
      1、客户端RPC请求NN，上传文件（写文件），NN校验请求（权限、目标是否存在），校验通过继续上传
      2、客户端首先请求上传第一个block连同其副本，
      3、NN根据副本策略（默认3副本）找出合适DN位置信息返回给客户端
      4、客户端根据DN位置信息，建立pipeline管道（client--->A--->B--->C）   ABC就是3个副本存储的DN机器
      5、客户端读取文件，以packet（默认64k）为大小往管道中发送数据
      6、在pipeline的反方向上，两两之间进行ACK校验
      7、当第一个block上传完毕，客户端继续请求NN上传后续的block 建立pipeline ack
      8、当所有的block上传完毕。客户端通知NN本次文件上传成功，NN记录文件的元数据信息。
      
      核心概念：pipeline（一次上传一个block 连同其所有副本）
      		ack(保证数据安全)
      		默认3副本策略
      ```
   ...
   2. #### HDFS读数据流程（下载）
      ...
      ```properties
      1、客户端RPC请求NN，下载文件（读文件），NN校验请求（权限、目标是否存在），校验通过
      2、NN视情况分批返回block位置信息
      	Q:视情况?  如果文件很大。blcok就多，分批读取数据。
         返回位置信息是有序的：
         		网络拓扑图中离客户端近的靠前
         		DN状态好的靠前  stale靠后
      3、客户端通过输入流到返回的数据块上read数据，在第一个读取完毕之前，连接到第二个块上继续读取
      4、当所有的数据块读取完毕，在客户端按照块的顺序进行拼接  并进行CRC校验。
      ```
